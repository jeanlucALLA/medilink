-- Script SQL pour configurer le cron job d'envoi automatique avec purge RGPD
...
SELECT * FROM cron.job WHERE jobname = 'send-delayed-emails-daily';
...
-- SELECT * FROM cron.job_run_details WHERE jobid = (SELECT jobid FROM cron.job WHERE jobname = 'send-delayed-emails-daily');
