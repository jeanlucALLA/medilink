...
-- Politique : Les utilisateurs peuvent lire leurs propres actes médicaux
CREATE POLICY "Users can view own medical acts"
  ON medical_acts FOR SELECT
  USING (auth.uid() = user_id);

-- Politique : Les utilisateurs peuvent insérer leurs propres actes médicaux
CREATE POLICY "Users can insert own medical acts"
  ON medical_acts FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Politique : Les utilisateurs peuvent mettre à jour leurs propres actes médicaux
CREATE POLICY "Users can update own medical acts"
  ON medical_acts FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- Politique : Les utilisateurs peuvent supprimer leurs propres actes médicaux
...
-- Créer un index sur act_date pour améliorer le tri et les requêtes par date
CREATE INDEX IF NOT EXISTS medical_acts_act_date_idx ON medical_acts(act_date DESC);
